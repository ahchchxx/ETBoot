package cn.exrick.xboot.common.generate;

import cn.exrick.xboot.common.exception.XbootException;
import cn.exrick.xboot.common.generate.bean.OrmField;
...
public class XbootVueGenerator {
    ...
    public static String generate(String template, boolean api, String vueName, Integer rowNum, List<OrmField> fields) throws IOException {
        // 模板路径
//        String root = System.getProperty("user.dir") + "/src/main/java/cn/exrick/xboot/generator/vue";
//        FileResourceLoader resourceLoader = new FileResourceLoader(root, "utf-8");
        ...
        // 排序
        Collections.sort(fields, Comparator.comparing(OrmField::getSortOrder));
        // 绑定变量
        tableTemplate.binding("api", api);
        //tableTemplate.binding("vueName", TbootMPGenerator.name(vueName, false));
        //tableTemplate.binding("apiName", TbootMPGenerator.name(vueName, true));
        // first letter lowercase
        tableTemplate.binding("vueName", vueName.substring(0, 1).toLowerCase() + vueName.substring(1));
        tableTemplate.binding("apiName", vueName);
        ...
        if("table.btl".equals(template)||"tableIndex.btl".equals(template)){
            ...
            List<OrmField> firstTwo = new ArrayList<>();
            List<OrmField> rest = new ArrayList<>();
            Integer count = 0;
            for(OrmField f:fields){
                if(f.getSearchable()){
                    count++;
    ...
    public static String gengerateEntityData(String path) throws Exception {
